...
fn_draw_exclusive <- function(plot_ready, V1, V2, cancer_types,p_val){
  ...
  pdf(file.path(snv_path,"mutual_exclusive_onlyICP",paste(fig_name,"pdf",sep = ".")),width = 4,height = 2)
  showtext_begin()
  p
  showtext_end()
  dev.off()
  # ggsave(file.path(snv_path,"mutual_exclusive",paste(fig_name,"pdf",sep=".")),device = "pdf",width=6,height=2)
  ggsave(file.path(snv_path,"mutual_exclusive_onlyICP",paste(fig_name,"png",sep = ".")),device = "png",width = 4,height = 2)
  
  graph2ppt(x = p,file = file.path(snv_path,"mutual_exclusive_onlyICP",paste(fig_name,"pptx",sep = ".")),width = 4,height = 2)
}
...
fn_mutal_exclusive <- function(cancer_types, ICP_SNV, cluster){
  combn(ICP_SNV$symbol,2) %>%
    t() %>% 
    as.data.frame() -> .gene_pairs
  ...
  .gene_pairs %>% 
    ...
    # multidplyr::cluster_assign_value("combine_data", ICP_SNV) %>%
    ...
    dplyr::mutate(rs = purrr::map2(V1, V2, .f = fn_ex, .data = ICP_SNV, cancer_types = cancer_types)) %>% 
    # dplyr::collect() %>%
    dplyr::as_tibble() %>%
    dplyr::ungroup() %>%
...
mutual_exclusive %>%
  readr::write_tsv(file.path(snv_path,"mutual_exclusive_onlyICP","ICP_mutual_exclusive.tsv"))