...
fn_draw_exclusive <- function(plot_ready, V1, V2, cancer_types,p_val){
  ...
  pdf(file.path(snv_path,"mutual_exclusive",paste(fig_name,"pdf",sep = ".")),width = 4,height = 2)
  showtext_begin()
  p
  showtext_end()
  dev.off()
  # ggsave(file.path(snv_path,"mutual_exclusive",paste(fig_name,"pdf",sep=".")),device = "pdf",width=6,height=2)
  ggsave(file.path(snv_path,"mutual_exclusive",paste(fig_name,"png",sep = ".")),device = "png",width = 4,height = 2)
  
  graph2ppt(x = p,file = file.path(snv_path,"mutual_exclusive",paste(fig_name,"pptx",sep = ".")),width = 4,height = 2)
}
