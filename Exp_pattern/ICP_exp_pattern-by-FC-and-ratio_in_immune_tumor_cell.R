...
fn_outlier_ratio <- function(cell_line_exp,.symbol){
  ...
  cellline_mid <- quantile(cell_line_exp$gene_tpm,0.8)
  immune_mid <- quantile(immune_exp$gene_tpm,0.2)
  tumor_ratio = length(which(cell_line_exp$gene_tpm>immune_mid))/length(cell_line_exp$gene_tpm)
  immune_ratio = length(which(immune_exp$gene_tpm>cellline_mid))/length(immune_exp$gene_tpm)
  
  FC %>%
    dplyr::mutate(tumor_ratio=tumor_ratio,immune_ratio=immune_ratio,`ratio_diff(T-N)`=tumor_ratio-immune_ratio)
}
...
fn_define_exp_site <- function(symbol,fc,pvalue,tumor_ratio,immune_ratio,ratio_diff,mean_cell_line, mean_immune_exp){
  print(symbol)
  if(is.na(pvalue)){
    tmp <- "Not_sure"
  } else {
    if(fc>=1 && pvalue<=0.05){
      if(tumor_ratio<=0.05){
        ...
        } else if(immune_ratio>=0.05 &&immune_ratio<0.25){
          tmp <- "Mainly_exp_on_Immune"
        }else{
          tmp <- "Both_exp_on_Tumor_Immune"
        }
      } else if(tumor_ratio>=0.05 && tumor_ratio<0.25){
        if(immune_ratio>=0.25){
          tmp <- "Mainly_exp_on_Immune"
        }else{
          tmp <- "Both_exp_on_Tumor_Immune"
      ...
      #     }
      #   }
      #  else if(ratio_diff>=0.5 && ratio_diff <0.95){
      #   tmp <- "Mainly_exp_on_Immune"
      # } else if(ratio_diff>=0.95){
      #   tmp <- "Only_exp_on_Immune"
      # } else if(ratio_diff<=(-0.5) && ratio_diff>(-0.95)){
      #   tmp <- "Mainly_exp_on_Tumor"
      # } else if(ratio_diff<=(-0.95)){
...
ICP_DE_FC_and_ratio_between_cellline_immune %>%
  dplyr::mutate(Exp_site = purrr::pmap(list(symbol,`log2FC(I/T)`,p.value,tumor_ratio,immune_ratio,`ratio_diff(T-N)`,mean_cell_line,mean_immune_exp),fn_define_exp_site)) %>%
