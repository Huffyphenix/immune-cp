##########################
# use xCell data from GTEx to confirm
# to confirm the conclusion that the TIL difference, and the TIL max in kidney normal tissue.
...
# calculate immune infitration of each normal tissues --------------------------------------
...
Immune_cells <- readr::read_tsv(file.path(basic_path,"data/TCGA/immune_infiltration/xCell","cell_type_to_class_xCell_score.tsv")) %>%
  dplyr::filter(Type == "Immune Cell")
## calculation
GTEx_expr.m <- as.matrix(GTEx_expr[,-c(1:2)])
rownames(GTEx_expr.m) <- GTEx_expr$Description

fn_xCell <- function(.x){
  exprMatrix = .x
  xCellAnalysis(exprMatrix)
}
## immune cells to calculated
# cell_list <- c("B-cells","CD4+ T-cells","CD8+ T-cells","aDC","iDC","DC","Neutrophils","Macrophages")

clinical_xcell_results <- fn_xCell(GTEx_expr.m)

GTEx_xCell <- file.path(basic_path,"data/TCGA/immune_infiltration/xCell")

GTEx_xCell %>%
  readr::write_rds(file.path(basic_path,"data/GTEx","GTEx_xCell-TIL.rds.gz"),compress = "gz")
