...
res_path <- file.path(immune_res_path,"TCGA_GSVAScore/all_togather")
score_path <- file.path(immune_res_path,"TCGA_GSVAScore")
...
GSVA.score.onlytumor <- 
  readr::read_rds(file.path(score_path,"TCGA.All-cancer-Tumor_GSVA.score_ICPs_features.rds.gz"))
...
GSVA.TIL.cor.DE.plot %>%
  ...
  geom_text_repel(aes(x=cor,y=FC,label=label,color=Features),data=label.data,size=3) +
  scale_color_manual(
...
GSVA.MB.cor.DE.plot %>%
  ggplot(aes(x=cor,y=FC)) +
  geom_jitter(aes(color=Features)) +
  geom_text_repel(aes(x=cor,y=FC,label=label,color=Features),data=label.data.meanMB,size=3) +
  scale_color_manual(
...
gene_list %>%
  ...
  tidyr::unnest() %>%
  dplyr::mutate(label = ifelse(n<5,Feature_symbol,Features)) %>%
  dplyr::inner_join(GSVA.CTL.DE.plot.all,by="Features") -> label.data.CTL

GSVA.CTL.DE.plot.all %>%
  ggplot(aes(x=Correlation,y=FC)) +
  geom_jitter(aes(color=Features)) +
  geom_text_repel(aes(x=Correlation,y=FC,label=label,color=Features),data=label.data.CTL,size=3) +
  scale_color_manual(
    values = c(cancer.color$color[1:nrow(GSVA.MB.cor.DE.plot)],cancer.color$color[1:nrow(GSVA.MB.cor.DE.plot)])
  ) +
...
GSVA_score_res.counts %>%
  ...
  ggplot(aes(x=name,y=Counts_of_significant_analysis_in_all_cancers)) +
  geom_bar(aes(color=color),stat="identity", fill="orange",alpha = 0.5,
           position=position_dodge()) +
  scale_x_discrete(limits=GSVA_score_res.counts$name) +
