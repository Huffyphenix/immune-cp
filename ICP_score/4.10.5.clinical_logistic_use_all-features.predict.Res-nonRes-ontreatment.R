################################
# construct logistics model for GSVA score of all possible features[include GSVA score, exp ratio, exp of ICPs]
...
for (times in 0:1000) {
  ...
  while(succss <= 10){
    ...
    predicted.classes <- ifelse(probabilities > 0.5, "yes", "no")
    observed.classes <- data.ready$Response
    res.auc.train <- roc(observed.classes, probabilities,quiet=TRUE)
    auc.train <- res.auc.train$auc[[1]]
    ...
    if(min(auc.train,auc.test)>=0.8){
      if(min(validation_auc$auc)>=0.7){
        ...
        ggsave(file.path(res_path,dir,paste("ROC_plot",succss,"png",sep=".")),device = "png",width = 5, height = 5)
        ggsave(file.path(res_path,dir,paste("ROC_plot",succss,"pdf",sep=".")),device = "pdf",width = 5, height = 5)
