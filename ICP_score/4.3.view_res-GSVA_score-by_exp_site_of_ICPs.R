...
# draw pic p---------------------------------------------------------------
fn_heatmap <- function(cancer_types, GSVA ){
  if(nrow(GSVA)>0){
  ...
  for (i in 2:5) {
    ...
    fn_survival.calculate(group, cancer_types, C, result_path = file.path(res_path, cancer_types), h = 3, w = 4)
    }
  }
}
...
fn_survival.calculate <- function(group, cancer_types, C, result_path, h, w){
  ...
  if (length(unique(.survival$group)) >= 2) {
    ...
    if (kmp <= 0.1) {
      ...
      color_list <- tibble::tibble(group = c(1:5),
                                   color = c("pink1", "skyblue1", "darkseagreen1", "darkgreen", "dodgerblue4"))
      sur_name <- paste("C", C, cancer_types, "GSVA_middle-survival-5y", sep = "_")
      result_path <- file.path(result_path,"survival")
