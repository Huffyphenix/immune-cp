...
gsva.score %>%
  dplyr::mutate(Cancer.y="All",blockade="All", Biopsy_Time="All", Author="All") %>%
  tidyr::nest(-Cancer.y,-blockade, -Biopsy_Time, -Author,.key = "GSVA") -> gsva.score.togather

gsva.score %>%
  dplyr::inner_join(Run_pubmed.id, by = "Run") %>%
  dplyr::select(-Response) %>%
  tidyr::nest(-Cancer.y,-blockade, -Biopsy_Time, -Author,.key = "GSVA") -> gsva.score.split
...
Run_pubmed.id %>%
  ...
  dplyr::inner_join(gsva.score.split, by = c("Cancer.y","blockade","Biopsy_Time","Author")) -> data_for_logistic.split
...
Run_pubmed.id %>%
  ...
  dplyr::mutate(Cancer.y="All",blockade="All", Biopsy_Time="All", Author="All") %>%
  tidyr::nest(Run,Response,.key = "response") %>%
  dplyr::inner_join(gsva.score.togather, by = c("Cancer.y","blockade","Biopsy_Time","Author")) -> data_for_logistic.togather

data_for_logistic.togather %>%
  rbind(data_for_logistic.split) -> data_for_logistic
...
data_for_logistic %>%
  dplyr::mutate(type = ifelse(Author=="All", "train", "validation")) %>%
  dplyr::mutate(n = purrr::map(GSVA,.f=function(.x){nrow(.x)})) %>%
  tidyr::unnest(n) %>%
...
fn_Run(stage=c("pre-treatment","on-treatment","All"),dir="all_as_train",test_n=10, train_n=20)
