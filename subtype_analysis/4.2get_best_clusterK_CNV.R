...
for (i in 2:20) {
  ...
  data.frame(sample = names(group),group=group,time = cnv_time,status = cnv_status) %>%
    dplyr::as.tbl() %>%
    dplyr::filter(group %in% more_than_10) %>%
    dplyr::rename("barcode"="sample") %>%
    dplyr::left_join(cnv_merge_snv_data.cancer_info,by="barcode") -> group_survival_data
  ...
  comp_list <- list()
  m_name <-  paste("CNV_mutation_for",C,"Clusters.png",sep="_")
  ...
  # Figure 4
  m_a_name <-  paste("CNV_all_mutation_for",C,"Clusters.png",sep="_")
