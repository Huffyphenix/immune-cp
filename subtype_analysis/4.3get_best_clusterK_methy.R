...
result_path <- file.path("/project/huff/huff/immune_checkpoint/result_20171025/subtype_analysis/methy/Get_best_clutser_20")
...
for (i in 2:20) {
  ...
  data.frame(sample = names(group),group=group,time = methy_time,status = methy_status) %>%
    dplyr::as.tbl() %>%
    dplyr::filter(group %in% more_than_10) %>%
    dplyr::rename("barcode"="sample") %>%
    dplyr::left_join(genelist_methy_mutaion_class.cancer_info,by="barcode") -> group_survival_data
  color_list = rainbow(length(more_than_10))
  title <- paste("methy Survival for",C,"Clusters",sep=" ")
  sur_name <-  paste("methy_Survival_for",C,"Clusters.png",sep="_")
  ...
  comp_list <- list()
  m_name <-  paste("methy_mutation_for",C,"Clusters.png",sep="_")
  ...
  # Figure 4
  m_a_name <-  paste("methy_all_mutation_for",C,"Clusters.png",sep="_")
