###########################################################
# combined cluster analysis for non-scaled exp, methylation clusters, and CNV raw data cluster results
###########################################################
...
expr_scaled_clsuer_4 <- readr::read_tsv(file.path(result_path,"expr/Get_best_clutser_20","4_clusters_group_info.tsv"))
methy_scaled_clsuer_6 <- readr::read_tsv(file.path(result_path,"methy/Get_best_clutser_20","6_clusters_group_info.tsv"))
...
ConsensusClusterPlus(combine_data.m, maxK=20, reps=500,pItem=0.8,pFeature=1, title="nonscale_data.recode.combined.ConsensusClusterplus", clusterAlg="hc",distance="pearson",seed=1262118388.71279, plot = "pdf") -> cc_res

cc_res %>% readr::write_rds(path = file.path(data_result_path, ".nonscale_data.recode.combined.ConsensusClusterplus.rds.gz"), compress = "gz")