...
results <- readr::read_rds(file.path("/project/huff/huff/immune_checkpoint/genelist_data","genelist_expr_CC_20.rds.gz"))

# get best K for cluster results  -----------------------------------------
source("/project/huff/huff/github/immune-cp/subtype_analysis/funtions_to_draw_pic.R")

result_path <- file.path("/project/huff/huff/immune_checkpoint/result_20171025/subtype_analysis/expr/Get_best_clutser_20")
...
for (i in 2:20) {
  ...
  data.frame(sample = names(group),group=group,time = expr_time,status = expr_status) %>%
    ...
    dplyr::left_join(gene_list_expr.cancer_info,by="barcode") -> group_survival_data
  ...
  comp_list <- list()
  m_name <-  paste("Expr_mutation_for",C,"Clusters.png",sep="_")
  ...
  # Figure 4
  m_a_name <-  paste("Expr_all_mutation_for",C,"Clusters.png",sep="_")
